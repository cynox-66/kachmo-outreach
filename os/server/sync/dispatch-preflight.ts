 ...
 *
 * This closes that gap without touching the dispatcher. Every entry in scheduled-queue.json is evaluated with the
 * same outreachBlock() every other channel uses — against the Postgres lead, the active Postgres suppression and the
 * Titan ledger — and any blocking finding refuses the whole run. It reads the same workspace files the dispatcher
 * reads in the next step, so what it approves is what gets sent.
 *
