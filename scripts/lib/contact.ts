import type { KachmoLead, SuppressionEntry, ContactProvenance } from './schema.js';
...
const BLOCKING_STATUSES = new Set(['DO_NOT_CONTACT', 'OPT_OUT', 'REPLIED_NO', 'UNSUBSCRIBED']);

/**
 * The single outreach block check used by every queue and every logging CLI.
...
export function outreachBlock(
  ...
  for (const [field, value] of [
    ...
    if (value && BLOCKING_STATUSES.has(value)) return { blocked: true, reason: `${field} is ${value}` };
  }
