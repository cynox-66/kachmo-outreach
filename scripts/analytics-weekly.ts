...
import { parseTracker, EMAIL_SENT_STATUSES, EMAIL_REPLY_STATUSES, EMAIL_POSITIVE_STATUSES } from './lib/email-state.js';
...
const ratio = (from: string, to: string, k: number, n: number): Ratio => ({
  from, to, k, n,
...
export function generateWeeklyReport() {
  ...
  const connected = new Set(attempts.filter(a => !['NO_ANSWER', 'VOICEMAIL', 'WRONG_NUMBER', 'GATEKEEPER'].includes(a.outcome)).map(a => a.tn));
  ...
  const waReplied = new Set(leads.filter(l => l.whatsapp_outreach_status === 'REPLIED' || (l.whatsapp_outreach_status === 'OPT_OUT' && waSent.has(l.target_number))).map(l => l.target_number));
  ...
  const meetings = union(new Set(leads.filter(l => l.meeting_status).map(l => l.target_number)), new Set([...tracker.values()].filter(r => ['CALL_BOOKED', 'PROPOSAL_SENT', 'WON'].includes(r.status)).map(r => r.target_number)));
  const proposals = union(new Set(leads.filter(l => l.proposal_status === 'SENT').map(l => l.target_number)), new Set([...tracker.values()].filter(r => ['PROPOSAL_SENT', 'WON'].includes(r.status)).map(r => r.target_number)));
  const won = union(new Set(leads.filter(l => l.deal_stage === 'WON').map(l => l.target_number)), new Set([...tracker.values()].filter(r => r.status === 'WON').map(r => r.target_number)));
  ...
  const archetypes = [...new Set(leads.map(l => l.archetype_id))].sort().map(id => {
    ...
    return { archetype: `${id}: ${ls[0].raw_archetype_id?.replace(/^\d+:\s*/, '') ?? ls[0].archetype_label}`, leads: ls.length, contacted: has(contacted), responded: has(responded), meetings: has(meetings), won: has(won) };
  });
  ...
  const wonDeals = leads.filter(l => l.deal_stage === 'WON').map(l => {
  ...
  for (const e of events) if (e.timestamp.slice(0, 10) >= weekStart && e.event_type !== 'QUALIFICATION_CHANGED' && e.event_type !== 'PRIORITY_CHANGED') weekEvents[e.event_type] = (weekEvents[e.event_type] ?? 0) + 1;
  ...
  const md = [
    `# Weekly Outbound Report: ${weekStart} → ${today}`,
    '',
    `> ${report.sources}`,
    `> ${report.note}`,
    '',
    ...
    '',
    '## Won deals',
    ...(wonDeals.length ? wonDeals.map(d => `- ${d.target} ${d.company}: ${d.value_as_recorded ?? 'value not recorded'} (arch ${d.archetype_id}, via ${d.channel ?? 'unknown'})`) : ['- None yet.']),
    '',
    '## This week',
    `- Emails sent (per tracker): ${emailsThisWeek}`,
    ...Object.entries(weekEvents).map(([k, v]) => `- ${k}: ${v}`),
    '',
  ].join('\n');
  writeFileSync(resolve(process.cwd(), 'WEEKLY_OUTBOUND_REPORT.md'), md, 'utf-8');

  console.log(`\n📊 Weekly report → WEEKLY_OUTBOUND_REPORT.md (contacted ${contacted.size}, responded ${responded.size}, meetings ${meetings.size}, won ${won.size})`);
  return report;
}
