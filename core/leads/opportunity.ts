import type { ChannelType, KachmoLead, SuppressionEntry } from './schema.js';
import { phoneEligibility, emailRoute, whatsappEligibility } from '../contact/provenance.js';
...
export const SCOPE_BY_ARCHETYPE: Record<string, { description: string; scope: string }> = {
...
export type OpportunityOutcome = 'BLOCKED' | 'DISQUALIFIED' | 'OUTREACH_READY' | 'EVALUATED';

/**
 * Opportunity, channel strategy, readiness and deterministic next action for one lead — written onto the lead
 * exactly as the engine always has. No I/O. `ledgerStatus` is the lead's OUTREACH_TRACKER.md status, if any.
 */
export function applyOpportunity(lead: KachmoLead, suppression: SuppressionEntry[], ledgerStatus: string | null): OpportunityOutcome {
  ...
  if (lead.research_state === 'DISQUALIFIED') return 'DISQUALIFIED';
  ...
  const touched =
    (ledgerStatus && ledgerStatus !== 'PENDING') ||
  ...
  return lead.research_state === 'OUTREACH_READY' ? 'OUTREACH_READY' : 'EVALUATED';
}
