import type { KachmoLead, LeadPriority } from '../leads/schema.js';
import { isUrl } from '../contact/provenance.js';
import { isGenericDecisionMaker } from '../qualification/gates.js';
...
/** A heuristic commercial-attractiveness score. It is NOT a conversion probability. */
...
/** Writes a score calculation onto a lead exactly as the engine always has. No I/O. */
export function applyScores(lead: KachmoLead, s: ScoreCalculation): { previousPriority: LeadPriority | null; changed: boolean } {
  const previousPriority = lead.lead_priority;
  ...
  return { previousPriority, changed: !!previousPriority && previousPriority !== s.priority };
}
